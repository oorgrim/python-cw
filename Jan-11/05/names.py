#ПРОГРАМИРОВАНИЕ - ТВОРЧЕСКАЯ ПРОФЕССИЯ
#match case
...
def get_names2(arg1, *array) -> list: # * означает, что все, что мы впихнем в качестве аргумента, будет запихнуто в массив(список)

    result = []
    
    if type(arg1) is int:
        array
        # соединить число + список и получить просто список 

    else: #в случае списка,  а не числа
        array = arg1
    for number in array:
        match number:
            case 0: result.append('ноль')
